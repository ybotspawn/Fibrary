...
module FActiveDirectory =
    type public FActiveDirectory() =
        // use patten matching to take in a type and a field and then return the approriate object from a generic encapsulated method
        ...
        let getAllDomainControllers () =
            ...
            Seq.cast dcs
            |> Seq.filter (fun (dc:DomainController) -> FComputer.FComputer().Pingable dc.Name )
